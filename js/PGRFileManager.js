...
function PGRFileManager(options)
{
	...
	{		
		...
		$.post("php/folders.php", {fetchDir:encodeURI(dirname)}, function(res){
			if (res.res == "OK") {
	...
	function readFolder()
	{
	    ...
	    var test = encodeURI(currentDir);
	    
		$.post("php/files.php?type=" + options.filesType, {dir:encodeURI(currentDir), test:test}, function(res){
			if (res.res == "OK") {			
	...
	function copyFiles()
	{
		...
		$.post("php/files.php?type=" + options.filesType, {fun:"copyFiles", dir:encodeURI(currentDir),toDir:encodeURI($dropDir.attr("directory")),files:files, test:test}, function(res){
			//alert(res.test);
			if (!res) return;
			if (res.res == "OK") {
			} else if (res.res == "ERROR") {
	...
	function deleteFiles()
	{
		...
		$.post("php/files.php?type=" + options.filesType, {fun:"deleteFiles", dir:encodeURI(currentDir),files:files, lastdeletefiles:lastdeletefiles, test:test}, function(res){
			if (res.res == "OK") {
				$("#fileList").html(prepareFilesContent(res.files, fileListType));
				//alert(res.test);
				self.initFiles();
	...
	function rotateImage90Clockwise()
	{
		...
		$.post("php/files.php?type=" + options.filesType, {fun:"rotateImage90Clockwise", dir:encodeURI(currentDir),filename:encodeURI(filename),test:test}, function(res){
			if (res.res == "OK") {
				//alert(res.test);
				$("#fileList").html(prepareFilesContent(res.files, fileListType));
	...
	function addDir()
	{
	    ...
	    buttons[_('Ok')] = function() {
			...
			$.post("php/folders.php", {fun:"addDir", dirname:encodeURI(dirname), newDirname:encodeURI(newDirname), fetchDir:encodeURI(dirname)}, function(res){
				if (res.res == "OK") {
				    initSubFolders($folder, res.folders);
	...
	{	    
	    ...
	    buttons[_('Ok')] = function() {
			...
			if (newDirname != dir.attr("dirname")) {
				...
				$.post("php/folders.php", {fun:"renameDir", dirname:encodeURI(dir.attr("directory")), newDirname:encodeURI(newDirname), fetchDir:encodeURI(parentFolderName)}, function(res){
					if (res.res == "OK") {
	...
	function moveDir()
	{
		...
		$.post("php/folders.php", {fun:"moveDir", dir:encodeURI($dir.attr("directory")),dirname:encodeURI($dir.attr("dirname")),toDir:encodeURI(dropDirName),fetchDir:encodeURI(dropDirName)}, function(res){
			if (res.res == "OK") {
	...
	function preview()
	{
		...
		if ($file.attr("fileType") == "image") {
			...
			var path = "userfiles" + currentDir + "/" + $file.attr("filename");
			//window.showModalDialog("test.php",path, "dialogHeight:500px; dialogWidth:600px; resizable:yes");
			$.post("php/files.php", {fun:"openFile", path:path}, function(res){
				if (res.res == "OK") {
					//path = decodeURI(res.path);			
					window.open(res.path);
					//window.showModalDialog(res.path,"", "dialogHeight:500px; dialogWidth:600px; resizable:yes"); 
					//window.showModelessDialog(res.path,"", "dialogHeight:500px; dialogWidth:600px; resizable:yes");
					//$openDir.mousedown();
					//$openDir.mouseup();
				} else if (res.res == "ERROR") {
					alert(_(res.msg));
				} else {
					alert(_("Can't open file"));				
				}
			}, "json");
		}
